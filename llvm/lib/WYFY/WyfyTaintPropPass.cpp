...
char WyfyTaintPropPass::ID = 0;
static RegisterPass<WyfyTaintPropPass> X("wyfy-taint", "Wyfy mark taint pass");

Pass *llvm::WYFY::createWyfyTaintPropPass() { return new WyfyTaintPropPass(); }

bool WyfyTaintPropPass::runOnModule(Module &M) {
  ...
  WyfyPtrAliasPass &MT = getAnalysis<WyfyPtrAliasPass>();
...
void WyfyTaintPropPass::getAnalysisUsage(AnalysisUsage &AU) const {
	AU.setPreservesAll();
  AU.addRequired<WyfyPtrAliasPass>();
}

map<Value *, WyfyPtrAliasPass::WyfyNode *> WyfyTaintPropPass::getValueMap() {
	return value_map;
}

void WyfyTaintPropPass::handleCmdLineArguments(Module &M) {
            ...
            for (auto pU: pV->users()) {
              if (GetElementPtrInst *pGEP = dyn_cast<GetElementPtrInst>(pU)) {
                ...
                for (auto pUb: pGEP->users()) {
                  if (LoadInst *pLI = dyn_cast<LoadInst>(pUb)) {
                    if (WyfyNode *_node = value_map[pGEP])
                      handleWyfyNode(_node);
                  }
                }
              }
            }
...
void WyfyTaintPropPass::handleInstructions(Module &M) {
  for (auto &F : M) {
    for (auto &BB : F) {
      for (auto &I : BB) {
				if (CallInst *pCI = dyn_cast<CallInst>(&I)) {
                  ...
                  } else if (auto pGEPOp = dyn_cast<GEPOperator>(arg)) {
                    // if the arg is GEP operator, then taint pointer operand.
                    Value *ptr = dyn_cast<Value>(pGEPOp->getPointerOperand());
                    if (WyfyNode *_node = value_map[ptr])
                      handleWyfyNode(_node);
                  } else {
                    pV->dump();
                    assert(false);
...
void WyfyTaintPropPass::handleWyfyNode(WyfyNode *node) {
...
void WyfyTaintPropPass::doTaintPropagation(Value *pV) {
...
Value *WyfyTaintPropPass::getArgument(Value *pI, Value *pV) {
	Function *pF;
...
void WyfyTaintPropPass::printNode(WyfyNode *node) {
  errs() << "Print node!\n";
	for (auto it = node->aliases.begin(); it != node->aliases.end(); it ++) {
		it->first->dump();
	}
...
void WyfyTaintPropPass::handleIntrinsicFunction(Function *pF, Value *pV, CallInst *pCI) {
	...
	if (pF->getName() == "memcpy" ||
		...
		if (pCI->getOperand(1) == pV) {
			...
			if (WyfyNode *dest_node = value_map[dest])
				handleWyfyNode(dest_node);
