...
void AArch64PassConfig::addIRPasses() {
  ...
  if (PARTS::getBeCfiType() == PARTS::PartsBeCfiFull)
	...
	//yh+begin
  const auto WyfyDpi = WYFY::getWyfyDpiType();

	if (WYFY::isEnableFuncCounter())
		addPass(WYFY::createWyfyFuncCounterPass());

	if (WyfyDpi != WYFY::WyfyNone) {
		addPass(WYFY::createWyfyPtrAliasPass());
		addPass(WYFY::createWyfyTaintPropPass());
		addPass(WYFY::createWyfyDataPtrSignPass());
	}
